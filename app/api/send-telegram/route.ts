  ...
  try {
        ...
        if (callbackUserId === MY_ADMIN_ID) {
          ...
          if (callbackData.startsWith('confirm_')) {
            ...
            if (order) {
              ...
              await fetch(`https://api.telegram.org/bot${botToken}/editMessageText`, { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ chat_id: chatId, message_id: messageId, text: oldText + "\n\n✅ **СТАТУС: ПОДТВЕРЖДЕНО**", parse_mode: "Markdown" })
              });
            }
          }
          ...
          if (callbackData.startsWith('decline_')) {
            ...
            if (order) {
              ...
              await fetch(`https://api.telegram.org/bot${botToken}/editMessageText`, { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ chat_id: chatId, message_id: messageId, text: oldText + "\n\n❌ **СТАТУС: НЕТ В НАЛИЧИИ**", parse_mode: "Markdown" })
              });
            }
          }
          ...
          if (callbackData.startsWith('ask_msg_')) {
            ...
            await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { 
              method: 'POST', headers: { 'Content-Type': 'application/json' }, 
              body: JSON.stringify({ chat_id: chatId, text: `📝 Напишите сообщение для заказа №${id}:\n(Используйте ОТВЕТ на это сообщение)`, reply_markup: { force_reply: true, selective: true } })
            });
          }
    ...
    if (body.message) {
      ...
      if (chatID === MY_ADMIN_ID) {
        if (text === '/admin') {
          ...
          if (!orders || orders.length === 0) {
            await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatID, text: "Заявок пока нет." }) });
          } else {
        ...
        if (body.message.reply_to_message) {
          ...
          if (idMatch && text.trim().length > 0) {
            ...
            if (order?.telegram_id) {
              await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ chat_id: Number(order.telegram_id), text: `💬 **Сообщение от менеджера:**\n\n${text}`, parse_mode: "Markdown" })
              });
              await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, 
      ...
      if (text.startsWith('/start')) {
        await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { 
          method: 'POST', headers: { 'Content-Type': 'application/json' }, 
          body: JSON.stringify({ chat_id: chatID, text: `✨ **Добро пожаловать!**`, parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🛵 Открыть каталог", web_app: { url: "https://scooter-danang.vercel.app" } }]] } })
        });
        return NextResponse.json({ ok: true });
      }
    }

    // --- 2. НОВЫЙ ЗАКАЗ (Из WebApp) ---
    // Проверяем наличие bike_model, чтобы понять, что это данные из каталога
    if (body.bike_model) {
      const { data: newOrder } = await supabase.from('bookings').insert([{ ...body, status: 'pending' }]).select().single();
      
      // Отправка уведомления именно тебе (админу)
      await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          chat_id: Number(MY_ADMIN_ID), 
          text: `🔔 **НОВЫЙ ЗАКАЗ №${newOrder?.id}**\nБайк: ${body.bike_model}\nКлиент: @${body.client_username || 'Не указан'}`, 
          parse_mode: 'Markdown',
