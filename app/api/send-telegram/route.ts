...
export async function POST(req: Request) {
  try {
    ...
    // Очищаем ID от возможных пробелов из настроек Vercel
    const MY_ADMIN_ID = String(process.env.TELEGRAM_CHAT_ID || '1920798985').trim();
    ...
    if (body.callback_query) {
      ...
      // Проверка управления заказами (только для админа)
          ...
          if (callbackData.startsWith('decline_')) {
            ...
            if (order) {
              ...
              await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { 
                ...
                body: JSON.stringify({
                  chat_id: Number(order.telegram_id),
                  text: `❌ **К сожалению, байка нет в наличии.**\nНапишите менеджеру для подбора замены.`,
                  parse_mode: "Markdown",
