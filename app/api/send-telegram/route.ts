...
export async function POST(req: Request) {
  try {
    ...
    // Используем переменные Vercel. Если их нет - уведомление не уйдет.
    const botToken = process.env.TELEGRAM_BOT_TOKEN;
    // Твой ID как число (без кавычек)
    const adminId = 1920798985; 
    
    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

    if (!botToken) return NextResponse.json({ error: "Token missing" }, { status: 500 });

    // --- ЛОГИКА 1: ОБРАБОТКА WEBHOOK ---
    if (body.message) {
    ...
    if (bike_model) {
      ...
      // 1. ОТПРАВЛЯЕМ АДМИНУ ПЕРВЫМ (Жестко на 1920798985)
      ...
      // 2. ОТПРАВЛЯЕМ КЛИЕНТУ (только если это не админ)
      ...
      return NextResponse.json({ success: true, admin_ok: adminResult.ok });
    }
