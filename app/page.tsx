...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 2. Инициализация Telegram и Реферальной системы
    const tg = (window as any).Telegram?.WebApp;
    if (tg) {
      tg.ready();
      tg.expand(); // Разворачиваем Mini App на весь экран
      
      // Проверяем реферала из Telegram (параметр start_param)
    ...
    // Проверка реферала из обычной ссылки (для тестов)
    ...
    // Подтягиваем реферала из памяти, если он был сохранен ранее
  ...
  const t = {
    ru: { 
      title: "Аренда скутеров", location: "Дананг, Вьетнам", sub: "DRAGON BIKE DANANG", 
      btn: "Забронировать", day: "в сутки", month: "в месяц"
    },
    en: { 
      title: "Scooter Rental", location: "Da Nang, Vietnam", sub: "DRAGON BIKE DANANG", 
  ...
  return (
    <main className="bg-[#05070a] min-h-screen text-white font-sans flex flex-col overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-[100] bg-[#05070a]/80 backdrop-blur-xl border-b border-white/5 h-20 flex items-center justify-between px-8">
        ...
        <button onClick={toggleLang} className="bg-white/5 border border-white/10 px-5 py-2 rounded-2xl text-[11px] font-bold uppercase active:scale-95 transition-all">
          {lang === 'ru' ? 'English' : 'Русский'}
        </button>
      </nav>

      {/* Hero Section */}
      <section className="relative h-[50vh] min-h-[400px] flex items-center justify-center text-center px-6 pt-16">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-400 text-lg font-light tracking-wide">{t[lang].location}</p>
          {ref && (
            <div className="mt-4 px-3 py-1 bg-white/5 border border-white/10 rounded-lg inline-block">
              <p className="text-[10px] text-green-500 uppercase font-bold tracking-widest">Partner: {ref}</p>
            </div>
          )}
        </div>
      </section>
      ...
      {/* Bikes Grid */}
      <section className="max-w-7xl mx-auto px-6 pb-24 -mt-12 relative z-20 flex-grow w-full">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {bikes.map((s) => {
              // Формируем ссылку на WhatsApp с учетом реферала
              const waMessage = `Hello! I want to book ${s.model} (${s.year}).${ref ? ` Agent: ${ref}` : ''}`;
              const waLink = `https://wa.me/${s.vendor_phone}?text=${encodeURIComponent(waMessage)}`;

              return (
                <div key={s.id} className="group bg-[#11141b] rounded-[2.5rem] border border-white/5 overflow-hidden transition-all hover:border-green-500/40">
                  ...
                  <div className="p-8 text-left">
                    ...
                    <div className="flex gap-3">
                      ...
                      <a href={waLink} target="_blank" className="flex-[1.5] bg-green-600 hover:bg-green-500 py-4 rounded-2xl font-bold text-[10px] uppercase text-center transition-all shadow-lg shadow-green-900/20">{t[lang].btn}</a>
                    </div>
                  </div>
                </div>
              );
