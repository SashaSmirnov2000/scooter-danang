...
export default function AdminPage() {
    ...
    async function fetchData() {
        ...
        try {
            if (activeTab === 'bikes') {
                ...
                const { data: bData } = await supabase.from('bookings').select('referrer, commission_amount').eq('status', 'completed');
                
                if (pData) {
                    const partnersWithStats = pData.map(p => {
                        const partnerBookings = bData?.filter(b => b.referrer === p.name) || [];
                        const totalBalance = partnerBookings.reduce((sum, b) => sum + (Number(b.commission_amount) || 0), 0);
                        return { ...p, paidCount: partnerBookings.length, balance: totalBalance };
                    });
    ...
    async function markAsPaid(id: string) {
        const amount = prompt("Введите сумму комиссии для партнера (VND):", COMMISSION_PER_ORDER.toString());
        if (amount === null) return;

        const { error } = await supabase.from('bookings').update({ 
            status: 'completed',
            commission_amount: parseInt(amount) || 0 
        }).eq('id', id);

        if (!error) fetchData();
    }

    async function payoutPartner(partnerName: string) {
        if (!confirm(`Обнулить баланс партнера ${partnerName}? Все его текущие оплаченные заявки будут помечены как выплаченные.`)) return;
        
        // Помечаем все завершенные заявки этого партнера как 'archived' или просто удаляем referrer, 
        // чтобы баланс обнулился. В данном случае обнуляем через обновление статуса заявок.
        const { error } = await supabase.from('bookings')
            .update({ referrer: `${partnerName}_paid` }) // Переименовываем реферера для сохранения истории, но исключения из баланса
            .eq('referrer', partnerName)
            .eq('status', 'completed');
                ...
                {activeTab === 'partners' && (
                    <div className="grid gap-4">
                        ...
                        {loading ? <p>Загрузка...</p> : partners.length === 0 ? <p className="text-gray-500 italic text-center">Партнеров пока нет</p> : partners.map(p => (
                            <div key={p.id} className="bg-[#11141b] p-6 rounded-[2rem] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                                <div>
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <button 
                                        onClick={() => payoutPartner(p.name)}
                                        className="bg-white/5 hover:bg-red-500/20 border border-white/10 hover:border-red-500/50 text-red-500 px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all"
                                    >
                                        Выплатить 💸
                                    </button>
                                </div>
